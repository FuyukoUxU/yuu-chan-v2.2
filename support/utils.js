...
module.exports = {
    getMember: async function(message, args = '', authorReturn = true ) {
        const a = await message.mentions.members.first() || message.guild.members.cache.get(args) || message.guild.members.cache.find(r => r.user.username.toLowerCase() === args.toLocaleLowerCase()) || message.guild.members.cache.find(r => r.displayName.toLowerCase() === args.toLocaleLowerCase()) || message.member;
        if(authorReturn) {
            return a
        } else {
            if(a == message.member) return undefined
            else return a
        }
    },
