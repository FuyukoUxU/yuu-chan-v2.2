module.exports = {
	name : 'quiz',
    description: 'Đố vui ( Câu hỏi tiếng anh )',
    usage: '_quiz <easy / medium / hard>',
    category: 'fun',
...
class Game {
    ...
    async get_data(dif, cat) {
        ...
        if (dif && !cat) {
            ...
            if (dif.toLowerCase() != 'easy' && dif.toLowerCase() != 'medium' && dif.toLowerCase() != 'hard') return this.message.channel.send('Hãy nhập độ khó hợp lệ\n easy / medium / hard');
        ...
        if (dif && cat) {
            ...
            if (dif.toLowerCase() != 'easy' && dif.toLowerCase() != 'medium' && dif.toLowerCase() != 'hard' && dif.toLowerCase() != 'any') return this.message.channel.send('Hãy nhập độ khó hợp lệ\n easy / medium / hard');
            if (!this.question_id) return this.message.channel.send('Nhập chủ đề hợp lệ');
    ...
    async await_reactions() {
        ...
        { max: 1, time: 30000 }).then(collected => {
            ...
            if (this.input_answer == this.correct_answer) {
                ...
                this.question_message.edit('Bạn đã trả lời đúng! :smile:')
                this.end_game()
            }
            else {
                this.answer_array[this.input_answer - 1] = this.answer_array[this.input_answer - 1] + ' ❌'
                ...
                .setFooter('Category - ' + atob(this.question.results[0].category) + ', Difficulty - ' + atob(this.question.results[0].difficulty))
                this.question_message.edit(this.question_embed)
                this.question_message.edit('Chúc bạn may mắn lần sau. Đáp án chính xác là ' + this.reactions[this.correct_answer - 1])
                this.end_game()
            }
        }).catch(() => {
            this.question_message.edit('Bạn mất quá nhiều thời gian để trả lời! Game Over và đáp án đúng là ' +  this.reactions[this.correct_answer - 1])
            this.end_game()
        })
    }
    async end_game() {
